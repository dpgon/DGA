...
class Random(object):
    """
    https://github.com/prodaft/malware-ioc/blob/master/FluBot/javarandom.py
...
class DGA:
    ...
    yara = "flubot.yar"
    name = "Flubot malware"
    ref = ["https://github.com/prodaft/malware-ioc/tree/master/FluBot",
           ...
           "https://www.incibe-cert.es/sites/default/files/contenidos/estudios/doc/incibe-cert_flubot_analysis_study_2021_v1.pdf"]
    desc = "Flubot is an Android banking malware."
    samples = 5000
    lcg = None
    use_date = 30
    use_seed = "Use a integer passed as a string. (1949 by default)"
    configs = ["2931", "1905", "1642", "1136", "1813", "1945", "1949"]
    variation = "Different domains and the tld is one of three possibilities."
    regex = "[a-y]{15}"
    ends = [".com", ".ru", ".cn", ".su"]

    def __init__(self, date=None, seed="1949"):
        self.date = date
        self.seed = int(seed)
        self.domain = ""
        self.generator = None
    ...
    def gen_seed(self):
        ...
        j5 = j4 * (0 ^ j4) & mask
        seed = (j5 + self.seed) & mask
        return seed

    def _dga(self):
        r = Random(seed=self.gen_seed())
        for i in range(5000):
            if self.domain.endswith(".com"):
                self.domain = self.domain.replace(".com", ".su")
                yield self.domain
            self.domain = ""
            for i2 in range(15):
                self.domain += chr(r.nextInt(25) + 97)
            if i % 3 == 0:
                self.domain += ".ru"
            elif i % 2 == 0:
                self.domain += ".com"
