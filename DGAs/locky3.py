class DGA:
    ...
    def _dga(self):
        seed_shifted = self.rol32(self.c['seed'], 17)
        dnr_shifted = self.rol32(self.nr, 21)
        k = 0
        year = self.date.year
        for _ in range(7):
            t_0 = self.ror32(0xB11924E1 * (year + k + 0x1BF5), self.c['shift']) & 0xFFFFFFFF
            t_1 = ((t_0 + 0x27100001) ^ k) & 0xFFFFFFFF
            t_2 = (self.ror32(0xB11924E1 * (t_1 + self.c['seed']), self.c['shift'])) & 0xFFFFFFFF
            t_3 = ((t_2 + 0x27100001) ^ t_1) & 0xFFFFFFFF
            t_4 = (self.ror32(0xB11924E1 * (self.date.day // 2 + t_3), self.c['shift'])) & 0xFFFFFFFF
            t_5 = (0xD8EFFFFF - t_4 + t_3) & 0xFFFFFFFF
            t_6 = (self.ror32(0xB11924E1 * (self.date.month + t_5 - 0x65CAD), self.c['shift'])) & 0xFFFFFFFF
            t_7 = (t_5 + t_6 + 0x27100001) & 0xFFFFFFFF
            t_8 = (self.ror32(0xB11924E1 * (t_7 + seed_shifted + dnr_shifted), self.c['shift'])) & 0xFFFFFFFF
        ...
        for i in range(length):
            k = (self.ror32(0xB11924E1 * self.rol32(k, i), self.c['shift']) + 0x27100001) & 0xFFFFFFFF
            self.domain += chr(k % 25 + ord('a'))
        self.domain += '.'
        k = self.ror32(k * 0xB11924E1, self.c['shift'])
        tlds = self.c['tlds']
